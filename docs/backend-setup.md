...
フォロー中・フォロワーの人数と一覧には **`supabase/migrations/20260913021456_profile_connections.sql` の追加適用が必要**。現時点ではローカルPGliteで検証済み、実環境への適用・マージ・デプロイは未実施。SQL未適用の実DBに接続すると一覧は取得エラーとなり、人数を「—」で表示する。既存のログイン・投稿・フォロー保存は引き続き既存RPCを使う。
