...
## フォロー一覧の追加SQL（2026-09-13）

ローカルの `.env.local` は既存RE:MEプロジェクトに設定し直し、所有者がログイン成功を確認した。値はGitに含めていない。

フォロー中・フォロワーの人数と一覧には `supabase/migrations/20260913021456_profile_connections.sql` を使用する。**既存RE:ME実環境では追加済み**。所有者から再実行時の `42P07: relation "follows_target_actor" already exists` 報告後、カタログの読み取りで索引定義・一覧RPCの本体・権限（anon不可、authenticated可）が実装と一致することを確認した。このSQLは再実行しない。次はローカルプロフィールを再読み込みして人数と一覧を操作確認する。マージ・デプロイは未実施。既存のログイン・投稿・フォロー保存は引き続き既存RPCを使う。

実環境への適用を決めた際の手順:

1. Supabaseで既存のRE:MEプロジェクトを開く。別プロジェクトを選ばない。
2. SQL Editorで上記の**追加SQLファイルだけ**を実行する。適用済みの初期SQLは再実行しない。アカウントや投稿の削除は不要。
3. ローカルのプロフィールを再読み込みし、「フォロー中」「フォロワー」を押す。2アカウント間のフォロー/解除と人数・一覧・プロフィール遷移を確認する。
4. ブロック後に相手が両方向から消えること、他者の一覧にも非表示の人が出ないことを確認する。週替わりはローカルテストで確認し、実環境で無断の全体リセットを実行しない。
5. SQLの適用履歴と確認結果を開発ログへ残す。サイト公開は別の明示的な作業とする。
...
- 公開API: `public.reme_state()`、`public.reme_mutate(action,payload,expected_epoch)`、追加SQL適用後の `public.reme_connections(target_public_id,expected_epoch)`。
- サーバーの`auth.uid()`と日本時間で本人・期間を判定。内部アカウントの対応表・メール・通報記録はレスポンスに含めない。
- 週替わり: 毎リクエストでJSTのISO週を算出するため、cronなしでも前週データを取得できない。新プロフィールを初回アクセス時に発行。
- 画面: 15秒間隔およびフォーカス復帰時に同期。サーバーが返す次の月曜0:00までの時間で画面を更新。ネットワーク遅延はあり得る。古い画面からの書き込みは`PERIOD_CHANGED`で拒否。
...
通常のリセットは毎週月曜0:00（JST）にDB時計で自動的に切り替わる。予定外の手動リセットを行う場合は参加者に予告し、所有者が `select reme_private.test_reset();` を実行する。**全参加者**の表示が切り替わる。利用停止や通報記録は継続する。一般利用者にはこの関数を呼ぶ権限がない。
...
- ブロックは週次の関係としてリセットされ、次週の相手との対応は利用者に公開しない。
